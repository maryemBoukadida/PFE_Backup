...
import { enregistrerFicheOlapion, envoyerFicheOlapion } from "./apiservices/api";
...
const initialoperation = [
  { specification:"", designation:'Etat du panneau avertisseur "Power Wizard"', matin:{normal:false,anomalie:false}, apresMidi:{normal:false,anomalie:false}, nuit:{normal:false,anomalie:false} },
  ...
  { specification:"", designation:"Niveau de gasoil dans le réservoir en %", matin:{normal:false,anomalie:false}, apresMidi:{normal:false,anomalie:false}, nuit:{normal:false,anomalie:false} },
  ...
  { specification:"", designation:"Etat  des conduites et de la pompe à gasoil", matin:{normal:false,anomalie:false}, apresMidi:{normal:false,anomalie:false}, nuit:{normal:false,anomalie:false} },
  { specification:"", designation:"Etat  de séparateur d'eau", matin:{normal:false,anomalie:false}, apresMidi:{normal:false,anomalie:false}, nuit:{normal:false,anomalie:false} },
  { specification:"", designation:"Etat des batteries et cosses", matin:{normal:false,anomalie:false}, apresMidi:{normal:false,anomalie:false}, nuit:{normal:false,anomalie:false} },
  { specification:"", designation:"Etat de disjoncteur GPE", matin:{normal:false,anomalie:false}, apresMidi:{normal:false,anomalie:false}, nuit:{normal:false,anomalie:false} },
  { specification:"", designation:"Etat de l'inverseur", matin:{normal:false,anomalie:false}, apresMidi:{normal:false,anomalie:false}, nuit:{normal:false,anomalie:false} },
  { specification:"", designation:"Horaire de GPE", matin:{normal:false,anomalie:false}, apresMidi:{normal:false,anomalie:false}, nuit:{normal:false,anomalie:false} },
...
export default function FicheOlapionForm() {
 ...
 const handleCheckbox = (index, period, type) => {
  const data = [...fiche.operations]; // ✅
  data[index][period][type] = !data[index][period][type];
  setFiche({ ...fiche, operations: data }); // ✅
};
...
const handleChangeNested = (section, field, value) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const res = await enregistrerFicheOlapion(fiche);
    setFicheId(res._id);
  ...
  const handleEnvoyer = async () => {
    ...
    await envoyerFicheOlapion(ficheId);
    alert("Fiche envoyée ✅");
  };
