...
const allRows = inspectionsData.flatMap((insp, index) =>
  insp.inspections.map((item, i) => ({ ...item, inspIndex: index, itemIndex: i }))
);
...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      console.log("DATA BACKEND:", data); // 🔥 pour vérifier
...
const handleAddRow = () => {
  ...
  // 🔥 Si aucune inspection existe encore
  if (newInspectionsData.length === 0) {
    newInspectionsData.push({
      inspections: []
    });
  }

  // 🔥 Ajouter une ligne vide dans le premier bloc
...
<td>
  {item.matin?.etat === "OK" ? null : (
    <input
      ...
      onChange={(e) => {
        const newInspectionsData = [...inspectionsData];
        newInspectionsData[index].inspections[i].matin.observation = e.target.value;
        setInspectionsData(newInspectionsData);
      }}
    />
  )}
...
<td>
  {item.matin?.etat === "OK" ? null : (
    <input
      ...
      onChange={(e) => {
        const newInspectionsData = [...inspectionsData];
        newInspectionsData[index].inspections[i].matin.intervention = e.target.value;
        setInspectionsData(newInspectionsData);
      }}
    />
  )}
